...
func verifyChecksum(filename string) bool {
	...
	if len(remainingBytes) >= 67 {
		...
		if err == nil {
			...
			return checksum == HashBytes(remainingBytes[68:])
		}
	}
...
func readJSON(meta Metadata, object interface{}, filename string) error {
	...
	if len(remainingBytes) >= 67 {
		...
		if err == nil && checksum != HashBytes(remainingBytes[68:]) {
	...
	return errors.AddContext(json.Unmarshal(remainingBytes, &object), "parsing the json object")
}
...
func LoadJSON(meta Metadata, object interface{}, filename string) error {
	...
	if err != nil {
		return errors.AddContext(err, fmt.Sprintf("other threads are using the filename %v", filename))
	}
...
func SaveJSON(meta Metadata, object interface{}, filename string) error {
	...
	checksum := HashBytes(objBytes)
	if err := enc.Encode(checksum); err != nil {
		return errors.AddContext(err, "unable to encode checksum")
	}
	_, err = buf.Write(objBytes)
	if err != nil {
		return errors.AddContext(err, "unable to write object bytes")
	}
	data := buf.Bytes()
