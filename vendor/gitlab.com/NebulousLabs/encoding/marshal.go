...
func Marshal(v interface{}) []byte {
	b := new(bytes.Buffer)
	_ = NewEncoder(b).Encode(v) // no error possible when using a bytes.Buffer
	return b.Bytes()
}
...
func WriteFile(filename string, v interface{}) (err error) {
	...
	if err != nil {
		err = errors.New("error while writing " + filename + ": " + err.Error())
	}
	return
}
...
func ReadFile(filename string, v interface{}) (err error) {
	...
	if err != nil {
		err = errors.New("error while reading " + filename + ": " + err.Error())
	}
	return
}
