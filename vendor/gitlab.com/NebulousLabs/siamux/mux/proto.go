...
// readBufferSize is the buffer size used when reading from the underlying tcp
// connection of a mux.
const readBufferSize = 32 * 1024 // 32 kib
...
func (m *Mux) initUpdateSettingsClient() error {
	...
	_, serverSettings, err := m.managedReadFrame(m.staticConn)
	if err != nil {
...
func (m *Mux) initUpdateSettingsServer() error {
	...
	_, clientSettings, err := m.managedReadFrame(m.staticConn)
	if err != nil {
...
func (m *Mux) managedReadFrame(r io.Reader) (int, frame, error) {
	// Grab settings.
	m.staticMu.Lock()
	aead, maxFrameSizePackets, packetSize := m.staticAead, m.settings.MaxFrameSizePackets, uint32(m.settings.RequestedPacketSize)
	m.staticMu.Unlock()

	// Read frame.
	maxEncryptedFrameSize := maxFrameSizePackets * packetSize
	n, f, _, err := readFrame(r, aead, maxEncryptedFrameSize, packetSize)
	if err != nil {
...
// written.
func (m *Mux) managedWrite(b []byte, s *Stream, timeout <-chan struct{}) (int, error) {
	id := s.staticID
	...
	for payload := buf.Next(maxPayload); len(payload) > 0; payload = buf.Next(maxPayload) {
		...
		if s.staticDone() && s.closedErr != nil {
			return written, s.closedErr
		}

		// Write the frame but respect the write deadline.
		var writeErr error
		...
		go func(payload []byte) {
			...
			_, writeErr = m.managedWriteFrame(f)
...
func (m *Mux) staticIsShutDown() bool {
	select {
	case <-m.staticShutdownChan:
		return true
	default:
	}
	return false
...
func (m *Mux) threadedReceiveData() {
	...
	// Start the loop.
	b := bufio.NewReaderSize(m.staticConn, readBufferSize)
	for {
		select {
		case <-m.staticCtx.Done():
		...
		readData, frame, err := m.managedReadFrame(b)
		...
		if !exists && m.staticIsShutDown() {
			// Ignore frames for unknown streams while shutting
			// down.
			m.staticMu.Unlock()
			continue
		} else if !exists {
