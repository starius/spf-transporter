...
const (
	...
	// of a SiaMux error. If set, the entire payload is an error string. Bit 0
...
const (
	...
	// correct value. Any time the siamux receives a frameErrorBadInit, an
	...
	// setup information to establish a connection between two SiaMux peers. If
	...
	// the timeout on the SiaMux connection. Keepalive frames only need to be
	...
	// the SiaMux and will not only contain the announcement of a new stream but
	// also the first bits of data in the stream.
	frameIDNewStream
)

// frame defines a single frame for sending data through the siamux session.
...
type frameHeader struct {
	...
	// set of reserved IDs are used for SiaMux communications.
	id uint32
