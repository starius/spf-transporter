...
import (
	...
	"gitlab.com/NebulousLabs/siamux/deps"
)
...
const (
	...
	// map.
	pruneClosedStreamInterval = 5 * time.Minute
	...
	closedStreamReuseTimeout = 5 * time.Minute
...
type (
	// Mux is the underlying multiplexer of the SiaMux package. It is used to split
...
func newMux(ctx context.Context, conn net.Conn, pubKey ED25519PublicKey, privKey ED25519SecretKey, outgoing bool, log *log.Logger, f closeCallback, t timeoutCallback, shutdownChan <-chan struct{}) (_ *Mux, err error) {
	...
	mux := &Mux{
		...
		staticShutdownChan:    shutdownChan,
...
func NewClientMux(ctx context.Context, conn net.Conn, expectedPubKey ED25519PublicKey, log *log.Logger, f closeCallback, t timeoutCallback, shutdownChan <-chan struct{}) (_ *Mux, err error) {
	// client doesn't need the pubKey and privKey since it doesn't need to sign
	// the encryptionResponse.
	mux, err := newMux(ctx, conn, ED25519PublicKey{}, ED25519SecretKey{}, true, log, f, t, shutdownChan)
	if err != nil {
...
func NewServerMux(ctx context.Context, conn net.Conn, pubKey ED25519PublicKey, privKey ED25519SecretKey, log *log.Logger, f closeCallback, t timeoutCallback, shutdownChan <-chan struct{}) (_ *Mux, err error) {
	// server doesn't need the expectedPubKey since the client's key is ephemeral.
	mux, err := newMux(ctx, conn, pubKey, privKey, false, log, f, t, shutdownChan)
	if err != nil {
...
func (m *Mux) threadedPruneClosedStreams() {
	for {
		...
		// Clean up the closed streams.
		closedStreams := make(map[uint32]time.Time)
		for frameID, closingTime := range m.closedStreams {
			// Delete streams by creating a new map, transferring
			// over the streams that are not deleted and then
			// replacing the map with the new one. That way the GC
			// can free up the old map which might have grown to a
			// very large size.
			if time.Since(closingTime) <= closedStreamReuseTimeout {
				closedStreams[frameID] = m.closedStreams[frameID]
			}
		}
		m.closedStreams = closedStreams

		// Clean up the regular streams.
