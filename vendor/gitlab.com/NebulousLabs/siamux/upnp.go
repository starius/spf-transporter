...
func (sm *SiaMux) managedForwardPort(port string, desc string) error {
...
func (sm *SiaMux) managedClearPort(port string) error {
	if build.Release == build.Testing {
		return nil
	}
...
// threadedForwardPorts forwards both the TCP and WS ports of the Siamux in a
// non-blocking way.
func (sm *SiaMux) managedForwardPorts(tcpAddr, wsAddr string) {
	// Forward TCP.
	sm.staticWG.Add(1)
	go func() {
		...
		err = sm.managedForwardPort(port, "SiaMux TCP")
		if err != nil {
	...
	go func() {
		...
		err = sm.managedForwardPort(port, "SiaMux WS")
		if err != nil {
...
// managedClearPorts clears the ports of the siamux.
func (sm *SiaMux) managedClearPorts() {
	// Clear TCP port.
	sm.staticWG.Add(1)
	go func() {
		defer sm.staticWG.Done()
